...
from character import modifier, proficiency_bonus
...
#   "ac":             int,
#   "initiative_mod": int,
#   "attacks":        [{name, bonus, damage, damage_type}],
#   "is_player":      False,
#   "conditions":     [],
#   "xp":             int  (optional)
# }
...
def build_enemy(name, hp, ac, attacks, initiative_mod=0, xp=0):
    """Convenience constructor for an enemy combatant dict."""
    return {
        "name":           name,
        "hp":             hp,
        "max_hp":         hp,
...
def resolve_attack(session, attacker_name, target_name, attack_bonus,
    ...
    # conditions that grant advantage to attacker
    for cond in CONDITIONS_WITH_ADVANTAGE_VS:
        if _has_condition(session, target_name, cond):
            advantage = True
            break

    # conditions that impose disadvantage on attacker
...
def enemy_attack(session, enemy_name, attack_index=0):
    """Resolve an enemy attack against the player combatant."""
    ...
    # sync damage back to session current_hp if player was hit
    if result["hit"]:
...
def handle_death_save(session):
    """Roll a death save and update the session death_save tallies.
    Returns the roll result plus outcome: 'stable', 'dead', 'ongoing', or 'revived'.
    """
...
def end_turn(session):
    """Advance to the next living combatant, skipping the dead.
    Returns the new current combatant, or None if combat should end.
    """
...
def combat_summary(session):
    """Return a snapshot of the current combat state suitable for DM narration."""
    order   = session["initiative_order"]
    current = gs.current_combatant(session)
    return {
        "round":        session["round"],
