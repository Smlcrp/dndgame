...
from character import empty_character, save_character
...
def build_abilities(data: dict, mods: list[dict]) -> dict:
    ...
    # Racial / feat / item bonuses from modifier list
    for mod in mods:
...
def build_ac(data: dict, abilities: dict, mods: list[dict]) -> int:
    ...
    for item in data.get("inventory", []):
        ...
        if armor_type in (1, 2, 3):   # light/medium/heavy
            base = defn.get("armorClass", 10)
            dex_cap = defn.get("stealthCheckDisadvantage")
            if armor_type == 1:        # light — full dex
                base_ac = base + dex_mod
            elif armor_type == 2:      # medium — max +2 dex
                base_ac = base + min(dex_mod, 2)
            elif armor_type == 3:      # heavy — no dex
                base_ac = base
        elif armor_type == 4:          # shield
            base_ac += 2

    # Bonus AC from modifiers (e.g. Unarmored Defense, magic items)
    for mod in mods:
...
def build_attacks(data: dict, abilities: dict) -> list[dict]:
    from character import proficiency_bonus, modifier
    ...
    # Custom actions (e.g. Unarmed Strike, racial abilities)
...
def import_from_ddb(url_or_id: str, cobalt_token: str = "") -> dict:
    ...
    data         = raw.get("data", raw)   # API wraps in {"data": {...}}
